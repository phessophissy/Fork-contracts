...
async function main() {
    ...
    let teamWallet = "0xa70b638B70154EdfCbb8DbbBd04900F328F32c35";
    let ctxAmount = ethers.utils.parseEther("418500");
    ...
    const calldatas = [
        abi.encode(["address", "uint256"], [teamWallet, ctxAmount]),
    ];
    const description =
        "CIP-39: Operating Expenditures Treasury Transfers for Q3 + Q4";
    console.log(targets);
    ...
    let ctxbalance = await ctxContract.balanceOf(teamWallet);
    console.log("team old CTX balance", ethers.utils.formatEther(ctxbalance));
    ...
    if (hardhatArguments.network === "hardhat") {
        ...
        ctxbalance = await ctxContract.balanceOf(teamWallet);
        console.log(
            "team new CTX balance",
